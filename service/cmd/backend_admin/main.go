...
import (
	...
	"github.com/andy2kuo/TourHelper/internal/server/backend_admin"
)

var SERVICE_NAME = "backend_admin_server" // 預設值，會在編譯時透過 -ldflags 覆寫
...
func main() {
	...
	logger.Infof("啟動 Backend Admin Server %s，版本 %s", SERVICE_NAME, SERVICE_VERSION)
	// 這裡會Block直到伺服器停止
	err := server.StartServer(&backend_admin.BackendAdminServer{}, opts)
	if err != nil {
		logger.Fatalf("Backend Admin Server 啟動失敗: %v", err)
	}
