...
import (
	...
	"github.com/andy2kuo/TourHelper/internal/server/backend"
)

var SERVICE_NAME = "backend_server" // 預設值，會在編譯時透過 -ldflags 覆寫
...
func main() {
	...
	logger.Infof("啟動 Backend Server %s，版本 %s", SERVICE_NAME, SERVICE_VERSION)
	// 這裡會Block直到伺服器停止
	err := server.StartServer(&backend.BackendServer{}, opts)
	if err != nil {
		logger.Fatalf("Backend Server 啟動失敗: %v", err)
	}
