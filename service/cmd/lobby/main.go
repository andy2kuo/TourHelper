...
var SERVICE_NAME = "lobby_server" // 預設值，會在編譯時透過 -ldflags 覆寫
...
func main() {
	...
	logger.Infof("啟動 Lobby Server %s，版本 %s", SERVICE_NAME, SERVICE_VERSION)

	// 建立 Lobby Server 實例
	lobbyServer := &lobby.LobbyServer{}

	// TODO: 如果需要與 Tour Server 雙向溝通，需要先啟動 Tour Server 並設定連線
	// 範例:
	// tourServer := &tour.TourServer{}
	// tourServer.Init(tourOpts)
	// go tourServer.Start()
	// lobbyServer.SetTourServer(tourServer)

	// 這裡會Block直到伺服器停止
	err := server.StartServer(lobbyServer, opts)
	if err != nil {
		logger.Fatalf("Lobby Server 啟動失敗: %v", err)
	}
