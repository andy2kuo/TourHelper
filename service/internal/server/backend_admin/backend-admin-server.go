...
// BackendAdminServer 後台管理伺服器,使用 Gin 框架
// 主要功能:後台管理功能,包含使用者登入驗證
type BackendAdminServer struct {
	router     *gin.Engine
	opt        *server.Options
...
// Init 初始化伺服器
func (s *BackendAdminServer) Init(opts *server.Options) error {
	// 設定 Gin 模式
...
// setupRoutes 設定所有路由
func (s *BackendAdminServer) setupRoutes() {
	// 健康檢查路由
	...
	logger.Info("Backend Admin 路由已設定完成")
}

// Start 啟動 HTTP 伺服器
func (s *BackendAdminServer) Start() error {
	...
	logger.Infof("Backend Admin 伺服器啟動於 %s", addr)
...
// Stop 停止 HTTP 伺服器
func (s *BackendAdminServer) Stop(ctx context.Context) error {
	if s.httpServer == nil {
		return nil
	}

	logger.Info("正在關閉 Backend Admin 伺服器...")
...
// Name 返回伺服器名稱
func (s *BackendAdminServer) Name() string {
	return "Backend Admin Server"
}
