@echo off
chcp 65001 >nul
REM TourHelper Lobby Service startup script
REM This script starts the lobby service with customizable service name, version, and environment
...
echo ====================================
echo TourHelper Lobby Service
echo ====================================
echo.

REM Prompt user for service name (default: lobby_server)
set /p SERVICE_NAME="Enter Service Name [lobby_server]: "
if "%SERVICE_NAME%"=="" set SERVICE_NAME=lobby_server
...
go run -ldflags "%LDFLAGS%" cmd\lobby\main.go
